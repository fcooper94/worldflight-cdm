...
/* Per-run removal cap. Defaults to 25 — enough for steady-state drift, small
 * enough that a bad read can't strip half the server. DISCORD_SYNC_MAX_REMOVALS
 * raises it temporarily for a first cleanup pass with a known-larger backlog;
 * drop the env var again afterwards. */
export const removalsCap = () => {
  const n = Number(process.env.DISCORD_SYNC_MAX_REMOVALS);
...
export function isDiffSafeToApply(diff, { minMatchRatio = 0.5, maxRemovals = removalsCap(), allowRemovals = removalsEnabled() } = {}) {
  const { stats } = diff;
  const reasons = [];
