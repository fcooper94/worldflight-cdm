  ...
  inFlight = (async () => {
    ...
    const data = {
      ...
      members: members.map(m => ({
        ...
        // Raw ids as well as names: matching on names is lossy (unknown or
        // duplicated names resolve wrongly), and callers comparing against
        // known role ids need these.
        roleIds: m.roles || [],
...
/* Create a plain grey role with no permissions — a label, nothing more. */
export async function createRole(name) {
  return call(`/guilds/${process.env.DISCORD_GUILD_ID}/roles`, {
    method: 'POST',
    body: JSON.stringify({ name, permissions: '0', color: 0, hoist: false, mentionable: false })
  });
}

/* Bulk reposition. Discord takes [{ id, position }] and re-ranks the rest. */
export async function setRolePositions(positions) {
...
/* Send a message to a channel by name. Channel ID is cached after first lookup.
   Accepts plain string content OR a full message payload object with embeds/components. */
const channelIdCache = {};
export async function sendChannelMessage(channelName, payload) {
  if (!discordConfigured()) return null;
  const guild = process.env.DISCORD_GUILD_ID;
  if (!channelIdCache[channelName]) {
    const channels = await call(`/guilds/${guild}/channels`);
    const ch = channels.find(c => c.name === channelName && (c.type === 0 || c.type === 5));
    if (!ch) throw new Error('Channel #' + channelName + ' not found');
    channelIdCache[channelName] = ch.id;
  }
  const body = typeof payload === 'string' ? { content: payload } : payload;
  return call(`/channels/${channelIdCache[channelName]}/messages`, {
    method: 'POST',
