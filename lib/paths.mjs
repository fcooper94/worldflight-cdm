...
/* vATIS template and per-airport profile overrides. Edited through the admin
   API at runtime, so they must outlive a deploy like the pack does. */
export const VATIS_DIR = VOL ? path.join(VOL, 'vatis') : path.join(ROOT, 'data', 'vatis');
...
export function ensurePackDirs() {
  for (const d of [PACK_DIR, GROUND_CACHE_DIR, PACK_OSM_CACHE_DIR, VATIS_DIR]) {
    try {
